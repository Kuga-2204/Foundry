...
export async function notifyFollowers(problemId, exceptUserId, type, message, link = "") {
  ...
  for (const f of followers) {
    ...
    seen.add(f.user_id);
    await notify(f.user_id, type, message, link);
  }
}

export async function follow(problemId, userId) {
  if (!userId) return;
  await db.prepare(
