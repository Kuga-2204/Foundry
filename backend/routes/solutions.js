...
async function attachMeta(solution, userId) {
  const agg = await db
    .prepare(
...
router.get("/problem/:problemId", optionalAuth, async (req, res) => {
  const problem = await db.prepare("SELECT id FROM problems WHERE id = ?").get(req.params.problemId);
  ...
  const withMeta = [];
  for (const r of rows) withMeta.push(await attachMeta(r, req.userId));
  res.json({
    solutions: withMeta,
    canReview: await hasStake(req.params.problemId, req.userId),
  });
});

// Post a solution, personally or on behalf of a startup you own.
router.post("/problem/:problemId", requireAuth, async (req, res) => {
  ...
  const problem = await db.prepare("SELECT * FROM problems WHERE id = ?").get(req.params.problemId);
  if (!problem) return res.status(404).json({ error: "Problem not found." });
  ...
  res.status(201).json({ solution: await attachMeta(row, req.userId) });
});
...
router.get("/:id", optionalAuth, async (req, res) => {
  ...
  res.json({ solution: await attachMeta(row, req.userId), canReview: await hasStake(row.problem_id, req.userId) });
});
